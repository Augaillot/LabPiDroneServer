...
class server:
	...
	def securityCheck(self):
		while self.started:
			...
			information["usb"] = []
			information["usb"].append('/home/pi')
		...
		if message["action"] == "record":
			...
			if information["record"] == False:
				if len(information["usb"]) == 1:
					if os.path.isdir(information["usb"][0]+'/Video') == False:
						os.mkdir(information["usb"][0]+'/Video')
					camera.start_recording(information["usb"][0]+"/Video/"+time.strftime("%y-%m-%d %H.%M")+".h264")
